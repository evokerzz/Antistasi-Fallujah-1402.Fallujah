...
if (!alive petros) then
	...
	if (_posicion distance getMarkerPos respawnBuenos > 999) then
		{
		["DEF_HQ",[format ["Enemy knows our HQ coordinates. They have sent a SpecOp Squad in order to kill al-Rashid. Intercept them and kill them. Or you may move our HQ 1Km away so they will loose track",name petros],format ["Defend %1",name petros],respawnBuenos],_posicion,"SUCCEEDED"] call A3A_fnc_taskUpdate;
