...
if (_marcador in puertos) then
	...
	["TaskSucceeded", ["", "Warehouse Taken"]] remoteExec ["BIS_fnc_showNotification",_winner];
	["TaskFailed", ["", "Warehouse Lost"]] remoteExec ["BIS_fnc_showNotification",_looser];
	["TaskUpdated",["",format ["%1 lost a Warehouse",_texto]]] remoteExec ["BIS_fnc_showNotification",_other];
	};
...
if (_winner == buenos) then
	...
	if (!isNull _bandera) then
		...
		if (_marcador in puertos) then {[_bandera,"Warehouse"] remoteExec ["A3A_fnc_flagaction",[buenos,civilian],_bandera]};
		};
