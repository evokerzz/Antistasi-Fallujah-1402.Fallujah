...
if (lados getVariable [_base,sideUnknown] == malos) then
	{
	if ((_base in puertos) and ([vehNATOLightArmed] call A3A_fnc_vehAvailable)) then
		{
		_tipoCoche = vehNATOLightArmed;
		_tipoPatrol = "LAND";
		}
	else
		{
	...
	{
	_lado = muyMalos;
	if ((_base in puertos) and ([vehCSATLightArmed] call A3A_fnc_vehAvailable)) then
		{
		_tipoCoche = vehCSATLightArmed;
		_tipoPatrol = "LAND";
		}
	else
		{
